-- Wrapper model that joins row titles with column config (Other Comprehensive Income
-- for the Year)
with
    titles as (
        select
...
left join
    column_cfg
    on column_cfg.eng_column_title = 'Other Comprehensive Income for the Year'
order by order_by
