-- Wrapper model that joins row titles with column config (Non-Distributable Profits)
with
    titles as (
        select
            order_by,
...
from titles
left join column_cfg on column_cfg.eng_column_title = 'Non-Distributable Profits'
order by order_by
