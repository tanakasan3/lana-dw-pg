select
    left(replace(nit_deudor, '-', ''), 14) as {{ ident('nit_deudor') }},
    left({{ ident('cod_cartera') }}, 2) as {{ ident('cod_cartera') }},
    left({{ ident('cod_activo') }}, 2) as {{ ident('cod_activo') }},
    left({{ ident('num_referencia') }}, 20) as {{ ident('num_referencia') }},
    to_char(round({{ ident('monto_referencia') }}, 2), 'FM9999999990.00') as {{ ident('monto_referencia') }},
    to_char(round({{ ident('saldo_referencia') }}, 2), 'FM9999999990.00') as {{ ident('saldo_referencia') }},
    to_char(round({{ ident('saldo_vigente_k') }}, 2), 'FM9999999990.00') as {{ ident('saldo_vigente_k') }},
    to_char(round({{ ident('saldo_vencido_k') }}, 2), 'FM9999999990.00') as {{ ident('saldo_vencido_k') }},
    to_char(round({{ ident('saldo_vigente_i') }}, 2), 'FM9999999990.00') as {{ ident('saldo_vigente_i') }},
    to_char(round({{ ident('saldo_vencido_i') }}, 2), 'FM9999999990.00') as {{ ident('saldo_vencido_i') }},
    to_char(round({{ ident('abono_deposito') }}, 2), 'FM9999999990.00') as {{ ident('abono_deposito') }},
    to_char( cast({{ ident('fecha_otorgamiento') }} as date), 'YYYY-MM-DD') as {{ ident('fecha_otorgamiento') }},
    to_char( cast({{ ident('fecha_vencimiento') }} as date), 'YYYY-MM-DD') as {{ ident('fecha_vencimiento') }},
    to_char( cast({{ ident('fecha_castigo') }} as date), 'YYYY-MM-DD') as {{ ident('fecha_castigo') }},
    left({{ ident('estado_credito') }}, 1) as {{ ident('estado_credito') }},
    to_char(round({{ ident('saldo_mora_k') }}, 2), 'FM9999999990.00') as {{ ident('saldo_mora_k') }},
    to_char(round({{ ident('saldo_mora_i') }}, 2), 'FM9999999990.00') as {{ ident('saldo_mora_i') }},
    cast({{ ident('dias_mora_k') }} as text) as {{ ident('dias_mora_k') }},
    cast({{ ident('dias_mora_i') }} as text) as {{ ident('dias_mora_i') }},
    to_char(
        cast({{ ident('fecha_inicio_mora_k') }} as date), 'YYYY-MM-DD'
    ) as {{ ident('fecha_inicio_mora_k') }},
    to_char(
        cast({{ ident('fecha_inicio_mora_i') }} as date), 'YYYY-MM-DD'
    ) as {{ ident('fecha_inicio_mora_i') }},
    left({{ ident('pago_capital') }}, 1) as {{ ident('pago_capital') }},
    left({{ ident('pago_interes') }}, 1) as {{ ident('pago_interes') }},
    cast({{ ident('periodo_gracia_k') }} as text) as {{ ident('periodo_gracia_k') }},
    cast({{ ident('periodo_gracia_i') }} as text) as {{ ident('periodo_gracia_i') }},
    left({{ ident('garante') }}, 10) as {{ ident('garante') }},
    left({{ ident('emisión') }}, 15) as {{ ident('emisión') }},
    cast({{ ident('pais_destino_credito') }} as text) as {{ ident('pais_destino_credito') }},
    left({{ ident('destino') }}, 6) as {{ ident('destino') }},
    left({{ ident('codigo_moneda') }}, 1) as {{ ident('codigo_moneda') }},
    to_char(round({{ ident('tasa_interes') }}, 2), 'FM9999999990.00') as {{ ident('tasa_interes') }},
    to_char(round({{ ident('tasa_contractual') }}, 2), 'FM9999999990.00') as {{ ident('tasa_contractual') }},
    to_char(round({{ ident('tasa_referencia') }}, 2), 'FM9999999990.00') as {{ ident('tasa_referencia') }},
    to_char(round({{ ident('tasa_efectiva') }}, 2), 'FM9999999990.00') as {{ ident('tasa_efectiva') }},
    left({{ ident('tipo_tasa_interes') }}, 1) as {{ ident('tipo_tasa_interes') }},
    left({{ ident('tipo_prestamo') }}, 2) as {{ ident('tipo_prestamo') }},
    left({{ ident('codigo_recurso') }}, 2) as {{ ident('codigo_recurso') }},
    to_char( cast({{ ident('ultima_fecha_venc') }} as date), 'YYYY-MM-DD') as {{ ident('ultima_fecha_venc') }},
    cast({{ ident('dias_prorroga') }} as text) as {{ ident('dias_prorroga') }},
    to_char(round({{ ident('monto_desembolsado') }}, 2), 'FM9999999990.00') as {{ ident('monto_desembolsado') }},
    left({{ ident('tipo_credito') }}, 2) as {{ ident('tipo_credito') }},
    to_char(
        cast({{ ident('fecha_ultimo_pago_k') }} as date), 'YYYY-MM-DD'
    ) as {{ ident('fecha_ultimo_pago_k') }},
    to_char(
        cast({{ ident('fecha_ultimo_pago_i') }} as date), 'YYYY-MM-DD'
    ) as {{ ident('fecha_ultimo_pago_i') }},
    cast({{ ident('dia_pago_k') }} as text) as {{ ident('dia_pago_k') }},
    cast({{ ident('dia_pago_i') }} as text) as {{ ident('dia_pago_i') }},
    cast({{ ident('cuota_mora_k') }} as text) as {{ ident('cuota_mora_k') }},
    cast({{ ident('cuota_mora_i') }} as text) as {{ ident('cuota_mora_i') }},
    to_char(round({{ ident('monto_cuota') }}, 2), 'FM9999999990.00') as {{ ident('monto_cuota') }},
    left({{ ident('cuenta_contable_k') }}, 12) as {{ ident('cuenta_contable_k') }},
    left({{ ident('cuenta_contable_i') }}, 12) as {{ ident('cuenta_contable_i') }},
    to_char( cast({{ ident('fecha_cancelacion') }} as date), 'YYYY-MM-DD') as {{ ident('fecha_cancelacion') }},
    to_char(round({{ ident('adelanto_capital') }}, 2), 'FM9999999990.00') as {{ ident('adelanto_capital') }},
    to_char(round({{ ident('riesgo_neto') }}, 2), 'FM9999999990.00') as {{ ident('riesgo_neto') }},
    to_char(round({{ ident('saldo_seguro') }}, 2), 'FM9999999990.00') as {{ ident('saldo_seguro') }},
    to_char(round({{ ident('saldo_costas_procesales') }}, 2), 'FM9999999990.00') as {{ ident('saldo_costas_procesales') }},
    left({{ ident('tipo_tarjeta_credito') }}, 1) as {{ ident('tipo_tarjeta_credito') }},
    left({{ ident('clase_tarjeta_credito') }}, 1) as {{ ident('clase_tarjeta_credito') }},
    left({{ ident('producto_tarjeta_credito') }}, 20) as {{ ident('producto_tarjeta_credito') }},
    to_char(round({{ ident('valor_garantia_cons') }}, 2), 'FM9999999990.00') as {{ ident('valor_garantia_cons') }},
    left({{ ident('distrito_otorgamiento') }}, 4) as {{ ident('distrito_otorgamiento') }},
    to_char(round({{ ident('reserva_referencia') }}, 2), 'FM9999999990.00') as {{ ident('reserva_referencia') }},
    left({{ ident('etapa_judicial') }}, 1) as {{ ident('etapa_judicial') }},
    to_char( cast({{ ident('fecha_demanda') }} as date), 'YYYY-MM-DD') as {{ ident('fecha_demanda') }},
    cast({{ ident('plazo_credito') }} as text) as {{ ident('plazo_credito') }},
    left({{ ident('orden_descuento') }}, 2) as {{ ident('orden_descuento') }},
    left({{ ident('categoria_riesgo_ref') }}, 2) as {{ ident('categoria_riesgo_ref') }},
    to_char(round({{ ident('reserva_constituir') }}, 2), 'FM9999999990.00') as {{ ident('reserva_constituir') }},
    to_char(round({{ ident('porcentaje_reserva') }}, 2), 'FM9999999990.00') as {{ ident('porcentaje_reserva') }},
    to_char(round({{ ident('pago_cuota') }}, 2), 'FM9999999990.00') as {{ ident('pago_cuota') }},
    to_char( cast({{ ident('fecha_pago') }} as date), 'YYYY-MM-DD') as {{ ident('fecha_pago') }},
    to_char(round({{ ident('porcenta_reserva_descon') }}, 2), 'FM9999999990.00') as {{ ident('porcenta_reserva_descon') }},
    to_char(round({{ ident('porcenta_adiciona_descon') }}, 2), 'FM9999999990.00') as {{ ident('porcenta_adiciona_descon') }},
    left({{ ident('depto_destino_credito') }}, 2) as {{ ident('depto_destino_credito') }},
    to_char(round({{ ident('porc_reserva_referencia') }}, 2), 'FM9999999990.00') as {{ ident('porc_reserva_referencia') }},
    to_char(round({{ ident('calculo_brecha') }}, 2), 'FM9999999990.00') as {{ ident('calculo_brecha') }},
    to_char(round({{ ident('ajuste_brecha') }}, 2), 'FM9999999990.00') as {{ ident('ajuste_brecha') }},
    left({{ ident('programa_asist_cafe') }}, 2) as {{ ident('programa_asist_cafe') }},
    to_char( cast({{ ident('fecha_cump_cafe') }} as date), 'YYYY-MM-DD') as {{ ident('fecha_cump_cafe') }}
from {{ ref("int_nrp_41_02_referencia") }}
